...
def main():
    ...
    assert_true(len(in_progress) == 0, "expected no Application In Progress records")
    assert_true({item.get("company") for item in in_progress} == set(), "unexpected Application In Progress companies")
    ...
    assert_true(next(item for item in applications if item.get("company") == "米哈游").get("appliedDate") == "2026-08-24", "miHoYo status transition missing")
    ...
    nio = next(item for item in applications if item.get("jobId") == "app-nio-hr-sparks-a73041-2026-09-07")
    ...
    mihoyo = next(item for item in applications if item.get("jobId") == "app-mihoyo-campus-position-9221-2026-08-23")
    meituan_status = next(item for item in applications if item.get("jobId") == "opp-single-url-4694828828")
    lenovo = next(item for item in applications if item.get("jobId") == "app-lenovo-campus-resume-2026-08-23")
    ...
    assert_true(nio.get("sourceJobId") == "A73041" and nio.get("appliedDate") == "2026-09-07" and not nio.get("jdUrl") and nio.get("currentStatus") == "Rejected" and nio.get("archived") is True, "NIO resume-screening rejection or link semantics missing")
    ...
    assert_true(meituan_status.get("currentStatus") == "Rejected" and meituan_status.get("archived") is True and "after an interview" in meituan_status.get("statusHistory", [])[-1].get("notes", ""), "Meituan interview rejection missing")
    assert_true(mihoyo.get("currentStatus") == "Rejected" and mihoyo.get("archived") is True and "after an interview" in mihoyo.get("statusHistory", [])[-1].get("notes", ""), "miHoYo interview rejection missing")
    assert_true(lenovo.get("currentStatus") == "Rejected" and lenovo.get("archived") is True and not lenovo.get("appliedDate") and "resume screening" in lenovo.get("statusHistory", [])[-1].get("notes", ""), "Lenovo resume-screening rejection must not invent an application date")
    assert_true(oppo.get("currentStatus") == "Rejected" and oppo.get("archived") is True and sum(event.get("status") == "Rejected" for event in oppo.get("statusHistory", [])) == 1 and "resume at screening" in next(event for event in oppo.get("statusHistory", []) if event.get("status") == "Rejected").get("notes", ""), "OPPO rejection must be stage-clarified without duplicate history")
    qwen = next(item for item in applications if item.get("company") == "阿里千问")
    assert_true(qwen.get("currentStatus") == "Rejected" and qwen.get("archived") is True and not qwen.get("appliedDate") and sum(event.get("status") == "Rejected" for event in qwen.get("statusHistory", [])) == 1 and "resume at screening" in next(event for event in qwen.get("statusHistory", []) if event.get("status") == "Rejected").get("notes", ""), "Qwen resume-screening rejection must not duplicate history or invent an application date")
